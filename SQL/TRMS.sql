...
BEGIN
    IF APPROVER = 'Supervisor' THEN
        IF REASON = 'Grade' AND (NEW_STATUS = 'Confirm' OR NEW_STATUS = 'Deny') THEN
            UPDATE R_FORMS SET APPROVE_GRADE = NEW_STATUS WHERE R_FORMS.ID = R_FORM_ID;
            IF NEW_STATUS = 'Confirm' THEN
                UPDATE R_FORMS SET FORM_STATUS = 'Approved' WHERE R_FORMS.ID = R_FORM_ID;
            ...
            ELSE
                UPDATE R_FORMS SET FORM_STATUS = 'Denied' WHERE R_FORMS.ID = R_FORM_ID;
            END IF;
        ELSE
    ...
    ELSIF APPROVER = 'Associate' THEN
        IF REASON = 'Pres' AND (NEW_STATUS = 'Confirm' OR NEW_STATUS = 'Deny') THEN
            UPDATE R_FORMS SET APPROVE_PRESENTATION = NEW_STATUS WHERE R_FORMS.ID = R_FORM_ID;
            IF NEW_STATUS = 'Confirm' THEN
                UPDATE R_FORMS SET FORM_STATUS = 'Approved' WHERE R_FORMS.ID = R_FORM_ID;
                UPDATE EMPLOYEES SET AVAILABLE_AMOUNT = (AVAILABLE_AMOUNT - (SELECT PENDING_REIMBURSEMENT FROM R_FORMS WHERE R_FORMS.ID= R_FORM)) WHERE EMPLOYEES.ID = (SELECT EMPLOYEE_ID FROM R_FORMS WHERE R_FORMS.ID = R_FORM);
            ELSE
        ...
        ELSE
            IF NEW_STATUS = 'Accept' THEN
                ...
                UPDATE R_FORMS SET FORM_STATUS = 'Pending' WHERE R_FORMS.ID = R_FORM_ID;
            ELSIF NEW_STATUS = 'Alternative' THEN
                UPDATE R_FORMS SET ALTERED_FORM = NEW_STATUS WHERE R_FORMS.ID = R_FORM_ID;
                UPDATE R_FORMS SET PENDING_REIMBURSEMENT = REASON WHERE R_FORMS.ID = R_FORM_ID;
            ELSE
                UPDATE R_FORMS SET ALTERED_FORM = NEW_STATUS WHERE R_FORMS.ID = R_FORM_ID;
...
FROM R_FORMS INNER JOIN EMPLOYEES ON (R_FORMS.ID = EMPLOYEES.ID)WHERE OFFICE_LOC ='Main' 
AND DEPARTMENT = 'IT' 
AND (r_forms.approve_supervisor = 'False' OR r_forms.form_status = 'Pending');

UPDATE EMPLOYEES SET AVAILABLE_AMOUNT = 1000;
...
CREATE SEQUENCE R_FORM_SEQ
  INCREMENT BY 1 MAXVALUE 5000 CYCLE;
EXECUTE INSERT_R_FORM(1,'01-MAY-2021','1 AM','Leader School',1000,800,'School for leader','I will lead better',1,'University Course',NULL,null);
EXECUTE INSERT_R_FORM(2,'01-MAY-2021','2 AM','Supervisor Seminar',900,700,'Seminar for Supervisors','It will help',2,'Seminar',NULL,null);
EXECUTE INSERT_R_FORM(3,'01-MAY-2021','3 PM','Associate prep class',800,600,'Prep class for associates','It will help',3,'Certification Prep Class',NULL,'True');
EXECUTE INSERT_R_FORM(3,'01-MAY-2021','4 PM','Associate Certification',600,500,'Certification for Associates','It will help',4,'Certification',NULL,null);
EXECUTE INSERT_R_FORM(3,'01-MAY-2021','5 PM','Associate Technical Training',500,400,'Technical Training for Associates','It will help',5,'Technical Training',NULL,null);
EXECUTE INSERT_R_FORM(3,'01-MAY-2021','6 PM','Associate therapy',400,300,'Therapy for Associates','It will help',5,'Other',NULL,'True');
...
select * from r_forms where is_urgent != null;
